...
export default function MentorProfileScreen() {
  ...
  const [sessionRequests, setSessionRequests] = useState(true);
  const [availableForMentorship, setAvailableForMentorship] = useState(user?.isAvailable !== false);
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <Card style={styles.profileCard}>
          ...
          <View style={styles.profileContent}>
            ...
            <Text style={styles.name}>{user?.name || [user?.firstName, user?.lastName].filter(Boolean).join(' ') || 'Mentor'}</Text>
            <Text style={styles.position}>{user?.currentPosition || user?.headline || 'Professional Mentor'}</Text>
            {user?.currentCompany && <Text style={styles.company}>{user.currentCompany}</Text>}
            
            <View style={styles.badges}>
              <Badge label={user?.role === 'alumni' ? 'Alumni' : 'Mentor'} variant="purple" />
              <Badge label={availableForMentorship ? 'Available' : 'Unavailable'} variant={availableForMentorship ? 'success' : 'error'} />
              {user?.isVerified && <Badge label="Verified" variant="success" icon={<Ionicons name="checkmark-circle" size={12} color={Colors.success} />} />}
            </View>

            <View style={styles.statsRow}>
              <View style={styles.statBox}>
                <Text style={styles.statValue}>{user?.menteesCount || 0}</Text>
                <Text style={styles.statLabel}>Mentees</Text>
              </View>
              <View style={styles.statDivider} />
              <View style={styles.statBox}>
                <Text style={styles.statValue}>{user?.sessionsCount || 0}</Text>
                <Text style={styles.statLabel}>Sessions</Text>
              </View>
              <View style={styles.statDivider} />
              <View style={styles.statBox}>
                <Text style={styles.statValue}>{user?.rating ? `${user.rating.toFixed(1)}⭐` : '5.0⭐'}</Text>
        ...
        <View style={styles.section}>
          ...
          <Card style={styles.settingsCard}>
            {renderSettingToggle('time-outline', 'Available for Sessions', Colors.success, availableForMentorship, setAvailableForMentorship)}
            ...
            <View style={styles.divider} />
            {renderSettingToggle('person-add-outline', 'Session Requests', Colors.warning, sessionRequests, setSessionRequests)}
          </Card>
        </View>
        ...
        <View style={styles.section}>
          ...
          <Card style={styles.settingsCard}>
            ...
            <TouchableOpacity style={styles.actionItem}>
              <View style={styles.settingLeft}>
                <View style={[styles.settingIconBox, { backgroundColor: `${Colors.primaryLight}20` }]}>
...
const styles = StyleSheet.create({
  ...
  position: { fontSize: FontSize.md, color: Colors.primaryLight, fontWeight: '500', marginTop: 3 },
  company: { fontSize: FontSize.sm, color: Colors.textSecondary, marginBottom: 8 },
