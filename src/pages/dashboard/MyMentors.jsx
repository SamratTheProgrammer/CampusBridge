              ...
              <div className="px-4 pb-4 sm:px-5 sm:pb-5 mt-2 sm:mt-4">
                <div className="grid grid-cols-3 gap-1.5 sm:gap-2">
                  <button 
                    ...
                    className="flex items-center justify-center gap-1 bg-background border border-border/50 hover:bg-muted text-foreground py-2 rounded-lg text-[11px] sm:text-xs font-medium transition-colors">
                    <User className="w-3 h-3 sm:w-3.5 sm:h-3.5 shrink-0" /> <span className="truncate">Profile</span>
                  </button>
                  <button 
                    onClick={() => navigate(`/dashboard/mentor/${mentor.clerkId}/book`)}
                    className="flex items-center justify-center gap-1 bg-primary hover:bg-primary/90 text-primary-foreground py-2 rounded-lg text-[11px] sm:text-xs font-medium transition-colors shadow-sm shadow-primary/20">
                    <Calendar className="w-3 h-3 sm:w-3.5 sm:h-3.5 shrink-0" /> <span className="truncate">Book</span>
                  </button>
                  <button className="flex items-center justify-center gap-1 bg-background border border-border/50 hover:bg-muted text-foreground py-2 rounded-lg text-[11px] sm:text-xs font-medium transition-colors">
                    <MessageSquare className="w-3 h-3 sm:w-3.5 sm:h-3.5 shrink-0" /> <span className="truncate">Chat</span>
                  </button>
                </div>
                <button 
                  onClick={() => handleUnfriend(mentor.id, mentor.name)}
                  className="w-full mt-2 flex items-center justify-center gap-1 bg-red-500/10 hover:bg-red-500 hover:text-white text-red-500 border border-red-500/20 py-1.5 sm:py-2 rounded-lg text-[11px] sm:text-xs font-medium transition-colors"
                >
                  Unfriend
                </button>
              </div>
